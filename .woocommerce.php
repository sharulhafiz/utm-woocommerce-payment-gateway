...
class cwoa_AuthorizeNet_AIM extends WC_Payment_Gateway {
	...
	function __construct() {
		...
		// Show Title
		$this->method_title = __( "UTM FPX", 'cwoa-authorizenet-aim' );

		// Show Description
		$this->method_description = __( "UTM FPX Payment Gateway Plug-in for WooCommerce", 'cwoa-authorizenet-aim' );

		// vertical tab title
		$this->title = __( "UTM FPX", 'cwoa-authorizenet-aim' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			// 'api_login' => array(
			// 	'title'		=> __( 'UTM FPX Login', 'cwoa-authorizenet-aim' ),
			// 	'type'		=> 'text',
			// 	'desc_tip'	=> __( 'This is the API Login provided by UTM FPX when you signed up for an account.', 'cwoa-authorizenet-aim' ),
			// ),
			// 'trans_key' => array(
			// 	'title'		=> __( 'UTM FPX Transaction Key', 'cwoa-authorizenet-aim' ),
			// 	'type'		=> 'password',
			// 	'desc_tip'	=> __( 'This is the Transaction Key provided by UTM FPX when you signed up for an account.', 'cwoa-authorizenet-aim' ),
			// ),
			'environment' => array(
				'title'		=> __( 'UTM FPX Test Mode', 'cwoa-authorizenet-aim' ),
	...
	public function process_payment( $order_id ) {
		...
		$environment_url = ( "FALSE" == $environment )
                               ? 'https://utmfpx.utm.my/fpx/01_new_fpx_main.php'
						   : 'https://utmfpx.utm.my/fpx/01_new_fpx_main.php';

		// This is where the fun stuff begins
		$payload = array(
			// UTM FPX Credentials and API Info
			// "x_tran_key"           	=> $this->trans_key,
			// "x_login"              	=> $this->api_login,
			// "x_version"            	=> "3.1",
      "desc"                => "Payment for UTM Marketplace order number ".$customer_order->get_order_number(),
      "systemType"          => 13,
      "payProgramCode"      => "UTMU001",

			// Order total
			"amount"             	=> $customer_order->order_total,
      "payWo"               => $customer_order->order_total,
      "payGST"              => $customer_order->order_total,
			...
			// Billing Information
			"nama"         	        => $customer_order->billing_first_name,
			...
			"icnum"            	=> $customer_order->user_id,
			// "x_customer_ip"        	=> $_SERVER['REMOTE_ADDR'],

		);

		// Send this payload to UTM FPX for processing
		...
		if ( empty( $response['body'] ) )
			throw new Exception( __( 'UTM FPX\'s Response was not get any data.', 'cwoa-authorizenet-aim' ) );
    ...
    echo $response_body;
		...
		if ( ( $r['response_code'] == 1 ) || ( $r['response_code'] == 4 ) ) {
			// Payment successful
			$customer_order->add_order_note( __( 'UTM FPX complete payment.', 'cwoa-authorizenet-aim' ) );
