 ...
 */
class WC_Gateway_UTM_FPX_Request {
	...
	/**
	 * Constructor.
	 * @param WC_Gateway_UTM_FPX $gateway
	 */
	public function __construct( $gateway ) {
		$this->gateway    = $gateway;
		$this->notify_url = WC()->api_request_url( 'WC_Gateway_UTM_FPX' );
	}
	...
	public function get_request_url( $order, $sandbox = false ) {
		...
		WC_Gateway_UTM_FPX::log( 'PayPal Request Args for order ' . $order->get_order_number() . ': ' . wc_print_r( $paypal_args, true ) );

		if ( $sandbox ) {
			return 'https://www.sandbox.paypal.com/cgi-bin/webscr?test_ipn=1&' . $paypal_args;
		} else {
			return 'https://utmfpx.utm.my/fpx/01_new_fpx_main.php?' . $paypal_args;
		}
	}
	...
	protected function get_paypal_args( $order ) {
		WC_Gateway_UTM_FPX::log( 'Generating payment form for order ' . $order->get_order_number() . '. Notify URL: ' . $this->notify_url );
