<?php
class WC_UTM_FPX extends WC_Payment_Gateway {
    public function __construct(){
      $this -> id = 'utmfpx'; // kena gantikan dengan FPX
      ...
      $this -> paymentapiid = $this -> settings['paymentapiid'];
      $this -> paymentapiuser = $this -> settings['paymentapiuser'];
      $this -> paymentapipass = $this -> settings['paymentapipass'];
      $this -> liveurl = 'https://utmfpx.utm.my/fpx/01_new_fpx_main.php';
	  //wak tambah -- validation key from UTM Digital	
    ...
    function init_form_fields(){
      $this -> form_fields = array(
        'enabled' => array(
            ...
            'default' => 'no'),
        'storename' => array(
            'title' => __('Store Name:', 'mrova'),
            'type'=> 'text',
            'description' => __('This will appear in payment receipt.', 'mrova'),
            'default' => __('Penerbit', 'mrova')),
        'title' => array(
        ...
        'description' => array(
            ...
            'default' => __('Pay securely by online transfer through UTM FPX Secure Servers.', 'mrova')),
        'paymentapiid' => array(
    ...
    public function admin_options(){
      ...
      echo '<p>'.__('UTM FPX is most popular payment gateway for online shopping in Malaysia').'</p>';
    ...
    public function generate_payu_form($order_id){
      ...
      global $woocommerce;
		...
		$order -> add_order_note('Customer redirected to UTM FPX');
      $redirect_url = ($this -> redirect_page_id=="" || $this -> redirect_page_id==0)?get_site_url() . "/":get_permalink($this -> redirect_page_id);

      $siteid = get_current_blog_id();
      if($siteid == 534){ //if ushop
        $idtabung = 13; // id tabung ushop = 13
        $productinfo = "Shop@UTM order " . $order_id;
      } else {
      ...
      return '<form action="'.$this -> liveurl.'" method="post" id="payu_payment_form">' . implode('', $payu_args_array) . '
            <input type="submit" class="button button-alt" id="submit_payu_payment_form" value="'.__('Pay via UTM FPX', 'mrova').'" /> <a class="button cancel" href="'.$order->get_cancel_order_url().'">'.__('Cancel order &amp; restore cart', 'mrova').'</a>
